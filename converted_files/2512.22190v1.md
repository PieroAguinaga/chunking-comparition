...
![ELIN test environment](page_6_image_4_v2.jpg)

![control scheme with acoustic sources](page_6_image_2_v2.jpg)
...
```mermaid
graph LR
    Agent -- "Action<br/>a<sub>t</sub>" --> Environment
    Environment -- "r<sub>t+1</sub>" --> Agent
    Environment -- "s<sub>t+1</sub>" --> Agent
    State_t["State<br/>s<sub>t</sub>"] --- Agent
    Reward_t["Reward<br/>r<sub>t</sub>"] --- Agent
    style Agent fill:none,stroke:#000
    style Environment fill:none,stroke:#000
```
...
To address this, an RL agent can be trained in sim-  ulation to determine the optimal energization instant  (i.e., circuit breaker closing angle) that minimizes  inrush current. The environment includes a trans-  former model receiving as inputs the three-phase remanent flux values (ϕ<sub>1</sub>, ϕ<sub>2</sub>, ϕ<sub>3</sub>) and the selected  switching angle (action). It outputs the peak inrush  current I<sub>max</sub>, which is used to compute the reward:

$$
r = \begin{cases} -I_{\text{max}}, & \text{if } I_{\text{max}} > 1, \\ 1 - I_{\text{max}}, & \text{otherwise.} \end{cases} \tag{10}
$$

The agent selects actions according to a policy π(a<sub>t</sub>|s<sub>t</sub>), often represented by a Neural Network, to maximize the expected cumulative reward over time. In most deep RL implementations, Neural Networks serve as policy approximators (actor), value estima- tors (critic), or both. These function approximators allow generalization across high-dimensional state spaces.
...
graph LR
    ...
    alpha["α closing"] --> TM
    flux["Φ₁, Φ₂, Φ₃"] --> TM
    TM --> Imax["I<sub>max</sub>"]
    style TM fill:none,stroke:#000
    style Environment fill:none,stroke:#000
```
